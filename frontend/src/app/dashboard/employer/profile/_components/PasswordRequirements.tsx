...
export function PasswordRequirements({ password }: PasswordRequirementsProps) {
    ...
    if (!password) {
        return (
            <div className="bg-gray-50/50 rounded-xl p-3 border border-gray-100 mt-2">
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Password Requirements</p>
                <div className="grid grid-cols-2 gap-y-2 gap-x-4">
                    {requirements.map((req, i) => (
                        <div key={i} className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] text-gray-500 font-medium">{req.label}</span>
                        </div>
                    ))}
                </div>
            </div>
    ...
    return (
        <div className="bg-emerald-50/30 rounded-xl p-3 border border-emerald-100/50 mt-2 transition-all duration-300">
            <p className="text-[11px] font-semibold text-emerald-800/60 uppercase tracking-wider mb-2">Password Requirements</p>
            <div className="grid grid-cols-2 gap-y-2 gap-x-4">
                {requirements.map((req, i) => {
